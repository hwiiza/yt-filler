...
見出しでセクションを区切ります。使用キーは `TITLE` / `DESCRIPTION` / `TAGS` / `THUMBNAIL`（`CHANNEL` / `NOTE` 等は無視）。**次の2形式どちらも読めます**：

- **A) インライン**: `==================== TITLE ====================`（`=` で見出しを囲む）
- **B) 罫線サンドイッチ**: 罫線行（`════` や `====`）→ 見出し行（` TITLE（…）` など）→ 罫線行 → 本文。`═`(U+2550) / `=` どちらの罫線でも可。
